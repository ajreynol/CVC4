...
std::ostream& operator<<(std::ostream& out, InferStep s)
{
  switch (s)
  {
    ...
    case InferStep::CHECK_NORMAL_FORMS_EQ:
      ...
      break;
    case InferStep::CHECK_MODEL_NORMAL_FORMS: out << "check_model_normal_forms"; break;
    ...
    case InferStep::CHECK_SEQUENCES_ARRAY_EAGER:
      ...
      break;
    case InferStep::RECHECK_FULL_IF_NO_MODEL: out << "recheck_full_if_no_model"; break;
...
void Strategy::initializeStrategy()
{
  ...
  if (!d_strategy_init)
  {
    ...
    {
      addStrategyStep(
          InferStep::CHECK_MODEL_NORMAL_FORMS);  // Only run if LOOKAHEAD pass.
                                      // Computes whether
    ...
    if (options().strings.stringModelBasedReduction)
    {
      ...
      if (options().strings.stringModelNormalForms)
      {
        addStrategyStep(InferStep::RECHECK_FULL_IF_NO_MODEL);
      }
      if (options().strings.stringExp)
      {
        addStrategyStep(InferStep::CHECK_EXTF_REDUCTION);
