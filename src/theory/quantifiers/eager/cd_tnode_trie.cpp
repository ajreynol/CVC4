...
CDTNodeTrie::CDTNodeTrie(context::Context* c)
    : d_data(c), d_repMap(c), d_repSize(c, 0), d_toMerge(c), d_toMergeProcessed(c, 0)
{
}
...
CDTNodeTrieAllocator::CDTNodeTrieAllocator(context::Context* c) : d_ctx(c), d_alloc(c) {}
...
CDTNodeTrieIterator::StackFrame::StackFrame(CDTNodeTrieAllocator* al, QuantifiersState& qs, CDTNodeTrie* active)
{
  d_active = active;
