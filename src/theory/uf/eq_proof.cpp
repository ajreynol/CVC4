    ...
    Node conclusion, std::vector<Node>& premises, CDProof* p) const
{
  ...
  //  --------------------------------------------------------------------- TR
  //                   (= (= t1 t2) false)
  Node constApp = nodeManager()->mkNode(Kind::EQUAL, constChildren);
                         ...
                         std::unordered_set<Node>& assumptions) const
{
  ...
  if (d_id == MERGED_THROUGH_EQUALITY)
  {
    ...
    if (d_node.getKind() != Kind::EQUAL)
    {
      ...
      if (d_node.getKind() == Kind::NOT)
      {
        ...
        conclusion =
            d_node[0].eqNode(nodeManager()->mkConst<bool>(false));
      }
      else
      {
        intro = ProofRule::TRUE_INTRO;
        conclusion =
            d_node.eqNode(nodeManager()->mkConst<bool>(true));
      }
  ...
  if (d_id == MERGED_THROUGH_CONSTANTS)
  {
    ...
    Node constApp = nodeManager()->mkNode(k, constChildren);
