...
class IntBlaster : protected EnvObj, public ProofGenerator
{
 ...
 public:
  ...
  /**
   * Recheck that `fact` is a conclusion that would be produced by this
   * int-blaster instance, either as a rewrite or as an auxiliary lemma.
   *
   * @return `fact` if it is a valid int-blaster conclusion, null otherwise.
   */
  Node checkProof(Node fact);
  /** identify */
 ...
 protected:
  ...
  /** Recheck a rewrite conclusion of the int-blaster. */
  Node checkRewriteProof(Node fact);

  /** Recheck a range-constraint lemma of the int-blaster. */
  Node checkRangeProof(Node fact);

  /** Recheck a bitwise-constraint lemma of the int-blaster. */
  Node checkBitwiseProof(Node fact);

  /** Return true if n is of the form 2^k and update k. */
  bool getPow2(TNode n, uint32_t& k) const;

  /**
   * Return true if n is of the form iextract(high, low, base), encoded as an
   * integer division/modulus pair, and update base/high/low.
   */
  bool getIExtract(TNode n, Node& base, uint32_t& high, uint32_t& low) const;

  /**
   * Infer the bit-width associated with a range-constrained integer term
   * generated by the int-blaster.
   */
  bool getRangeConstraintBitWidth(TNode term, uint32_t& bvsize) const;
