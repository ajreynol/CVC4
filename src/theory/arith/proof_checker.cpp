                                          ...
                                          const std::vector<Node>& args)
{
  ...
  switch (id)
  {
    ...
    case ProofRule::ARITH_POLY_NORM_REL:
    {
      ...
      Node l = children[0][0];
      Node r = children[0][1];
      bool isBv = l.getType().isBitVector();
      Kind mkind = isBv ? Kind::BITVECTOR_MULT : Kind::MULT;
      if (l.getKind() != mkind || r.getKind() != mkind)
      {
        AlwaysAssert(false);
        return Node::null();
      }
      Node lr = l[1];
      Node rr = r[1];
      Kind skind = isBv ? Kind::BITVECTOR_SUB : Kind::SUB;
      if (lr.getKind() != skind || rr.getKind() != skind)
      {
        AlwaysAssert(false);
        return Node::null();
      }
      ...
      if (isBv)
      {
        ...
        if (c1 != one || c2 != one)
        {
        AlwaysAssert(false) << cx << " " << cy << " " << one << " " << children[0];
          return Node::null();
        }
      }
      else
