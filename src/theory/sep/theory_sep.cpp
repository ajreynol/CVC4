...
void TheorySep::initializeHeapTypes()
{
  if (d_env.hasSepHeap())
  {
    ...
    Trace("sep-type") << "Sep: assume location type " << d_type_ref
                      ...
                      << std::endl;
    d_nil_ref =
        nodeManager()->mkNullaryOperator(d_type_ref, Kind::SEP_NIL);
                                 ...
                                 unsigned ind)
{
  ...
  else
  {
    ...
    if (n.getKind() == Kind::SEP_STAR || n.getKind() == Kind::SEP_WAND)
    {
      if( lbl==o_lbl ){
        ...
        if (n.getKind() == Kind::SEP_STAR)
        {
          ...
          conj.push_back(
              nodeManager()->mkNode(Kind::AND, bchildren));
    ...
    {
      return lbl_v.eqNode(
          nodeManager()->mkConst(EmptySet(lbl_v.getType())));
    }else{
      std::map< Node, Node >::iterator it = visited.find( n );
      if( it==visited.end() ){
        std::vector< Node > children;
...
Node TheorySep::HeapInfo::getValue( TypeNode tn ) {
  ...
  if( d_heap_locs.empty() ){
    return nodeManager()->mkConst(EmptySet(tn));
  }
  Node curr = d_heap_locs[0];
  for (unsigned j = 1; j < d_heap_locs.size(); j++)
  {
    curr =
        nodeManager()->mkNode(Kind::SET_UNION, d_heap_locs[j], curr);
  }
  return curr;
}
