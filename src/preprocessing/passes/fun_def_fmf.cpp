...
void FunDefFmf::process(AssertionPipeline* assertionsToPreprocess)
{
  ...
  for (size_t i = 0, asize = assertions.size(); i < asize; i++)
  {
    ...
    if (!n.isNull())
    {
      ...
      if (!bd.isNull())
      {
        ...
        new_q = rewrite(new_q);
        assertionsToPreprocess->replace(i, new_q, nullptr, TrustId::PREPROCESS_FUN_DEF_FMF);
  ...
  for (size_t i = 0, asize = assertions.size(); i < asize; i++)
  {
    ...
    if (n != assertions[i])
    {
      ...
      assertionsToPreprocess->replace(i, n, nullptr, TrustId::PREPROCESS_FUN_DEF_FMF);
    }
  }
}
